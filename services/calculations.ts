...
export function generateVendorPurchaseOrders(lines: QuoteLine[], projectNumber: string, startingSequence = 5100): PurchaseOrder[] {
  return Object.entries(groupQuoteLinesByVendor(lines.filter(line => line.approved))).map(([vendor, vendorLines], index) => {
    const poNumber = `${projectNumber}-PO-${startingSequence + index + 1}`
