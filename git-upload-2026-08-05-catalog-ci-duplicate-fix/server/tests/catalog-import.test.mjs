...
test('catalog snapshot pagination uses a stable order before applying ranges', () => {
  const source = fs.readFileSync(new URL('../catalog-api.mjs', import.meta.url), 'utf8')
  assert.match(
    source,
    /select\('id,manufacturer,manufacturer_part_number,current_cost'\)[\s\S]*?\.order\('id',[\s\S]*?\.range\(from, from \+ 999\)/,
  )
})
