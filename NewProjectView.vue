<template>
  <div class="form-page">
    ...
    <form class="project-form" @submit.prevent="handleSubmit">
      ...
      <FormField
        v-if="form.projectType === 'Managed Funds'"
        v-model.number="form.checkbookStartingBalance"
        label="Original Authorized Funding"
        placeholder="0.00"
        type="number"
        step="0.01"
        min="0"
...
import { searchCustomerSuggestions, applyCustomerAddressToProjectInput, syncCustomersFromProjects } from '../services/customerRecords'
import { loadProjects, saveProject, saveProjectStrict } from '../services/localProjects'
...
async function handleSubmit() {
  ...
  if (input.projectType === 'Managed Funds' && input.checkbookStartingBalance <= 0) {
    window.alert('Managed Funds projects need a starting customer balance.')
    return
  }

  try {
    const saved = input.projectType === 'Managed Funds' ? await saveProjectStrict(input) : saveProject(input)
    router.push(`/projects/${saved.id}`)
  } catch (error) { window.alert(error instanceof Error ? error.message : 'Unable to save project.') }
}
