...
export async function saveRemoteRecord<T>(recordType: string, recordKey: string, data: T) {
  ...
  if (recordType === 'projects' && recordKey === 'all' && Array.isArray(data)) {
    const remote = await loadRemoteRecord<Project[]>('projects', 'all')
    if (data.some(isManagedFunds) || remote?.some(isManagedFunds)) {
      const clean = (p: Project) => { const { managedFunds, ...rest } = p; return rest }
      for (const old of remote || []) {
        if (isManagedFunds(old) && !data.some(p => p.id === old.id)) throw new Error('Managed Funds projects cannot be deleted. Retain their financial history.')
        if (isManagedFunds(old) && data.some(p => p.id === old.id && !isManagedFunds(p))) throw new Error('Managed Funds projects cannot change their type.')
      }
      for (const project of data.filter(isManagedFunds) as Project[]) {
        const old = remote?.find(p => p.id === project.id)
        if (!old || JSON.stringify(clean(old)) !== JSON.stringify(clean(project))) await saveFundsProject(project)
      }
      const { data: saved, error } = await supabase.rpc('atlas_merge_standard_projects', { p_projects: data.filter(p => !isManagedFunds(p)) })
