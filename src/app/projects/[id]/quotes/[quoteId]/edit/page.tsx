...
import { BadgeDollarSign, CheckCircle2, Download, FileSpreadsheet, FileUp, Plus, Save, Send, Upload } from "lucide-react";
...
import { exportCustomerQuotePdf } from "@/lib/customer-quote-pdf";
import { loadProjectShared, updateQuoteForProjectShared } from "@/lib/project-api-client";
...
import type { CustomerQuote, Project, QuoteLine } from "@/lib/types";
...
export default function EditProjectQuotePage() {
  const params = useParams<{ id: string; quoteId: string }>();
  const router = useRouter();
  const [project, setProject] = useState<Project | undefined>();
  const [quote, setQuote] = useState<CustomerQuote | undefined>();
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const nextQuote = nextProject?.quotes?.find((item) => item.id === params.quoteId);
        const nextShowPricingControls = nextProject?.projectType !== "Design & Install";
        setProject(nextProject);
        setQuote(nextQuote);
        setDraftLines(normalizePricingForProject(applySequentialClins(nextQuote?.lines ?? []), nextShowPricingControls));
        setExpirationDays(nextQuote?.expirationDays ?? 30);
        setContractFeeEnabled(nextQuote?.contractFeeEnabled ?? false);
        setShippingCost(nextQuote?.shippingCost ?? 0);
      } catch (error) {
        if (active) setSaveError(error instanceof Error ? error.message : "Unable to load this quote.");
      } finally {
  ...
  function handleAddLine(event: FormEvent<HTMLFormElement>) {
    ...
    setDraftLines((current) =>
      applySequentialClins([
        ...current,
        buildDraftLine({
          ...form,
          clin: getNextClin(current),
          quantity,
          unitCost,
          pricingMode: showPricingControls ? pricingMode : "markup",
          marginPercent: showPricingControls ? marginPercent : 0,
  ...
  async function handleImport(file: File | undefined) {
    ...
    try {
      ...
      ].map((line, index) => ({ ...line, clin: String(index + 1) })));
      setImportStatus(
        result.lineCount
          ? `Added ${result.lineCount} imported line item(s) to this quote.`
  ...
  async function saveQuote() {
    ...
    try {
      const updatedQuote = await updateQuoteForProjectShared(
        params.id,
        params.quoteId,
        normalizePricingForProject(applySequentialClins(draftLines), showPricingControls),
        { contractFeeEnabled, expirationDays, shippingCost }
      );
      router.push(`/projects/${params.id}?quote=${encodeURIComponent(updatedQuote.quoteNumber)}`);
    } catch (error) {
      setSaveError(error instanceof Error ? error.message : "Unable to save quote changes.");
    } finally {
      setIsSaving(false);
    }
  }
  ...
  async function importRfqResponse(file: File | undefined) {
    ...
    try {
      ...
      setRfqStatus(
        `Updated ${result.updatedCount} line${result.updatedCount === 1 ? "" : "s"} with vendor pricing. ${result.unmatchedCount} response line${result.unmatchedCount === 1 ? "" : "s"} did not match this quote.`
      );
    } catch (error) {
  ...
  if (loaded && (!project || !quote)) {
    return (
      <div className="space-y-4">
        <h2 className="text-2xl font-bold text-[#06163d]">Quote not found</h2>
        <Link href={`/projects/${params.id}`} className="font-bold text-[#0067e6]">Back to Project</Link>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-[#06163d]">Edit Quote {quote.quoteNumber}</h2>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button type="button" onClick={exportPdf} className="inline-flex items-center gap-2 rounded-md border border-[#dfe5ee] bg-white px-4 py-2 text-sm font-bold text-[#071b49]">
            <Download size={17} />Export PDF
          </button>
          <button type="button" onClick={exportRfqPackage} disabled={!draftLines.length} className="inline-flex items-center gap-2 rounded-md border border-[#dfe5ee] bg-white px-4 py-2 text-sm font-bold text-[#071b49] disabled:cursor-not-allowed disabled:text-[#9aa7ba]">
            <FileSpreadsheet size={17} />Vendor RFQ Workbooks
          </button>
          <button type="button" onClick={saveQuote} disabled={isSaving} className="inline-flex items-center gap-2 rounded-md bg-[#155fdb] px-4 py-2 text-sm font-bold text-white disabled:cursor-not-allowed disabled:bg-[#9db8e4]">
      ...
      <section className="rounded-[8px] border border-dashed border-[#b9c6d8] bg-[#f8fbff] p-4">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-xs font-semibold text-[#526179]">Upload an exported ROM Tool quote, Excel, CSV, TXT, or PDF to append material lines to this quote.</p>
          </div>
          <label className="inline-flex cursor-pointer items-center gap-2 rounded-md bg-white px-4 py-2 text-sm font-bold text-[#071b49] ring-1 ring-[#dfe5ee]">
            ...
            <input type="file" accept=".xlsx,.xls,.csv,.txt,.pdf,application/pdf,application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" className="hidden" onChange={(event) => void handleImport(event.target.files?.[0])} />
          </label>
        </div>
      ...
      <section className="space-y-4 rounded-[8px] border border-[#dfe5ee] bg-white p-5 shadow-[0_2px_8px_rgba(15,23,42,0.05)]">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm font-semibold text-[#526179]">
              Verify catalog pricing through vendor RFQs before issuing purchase orders.
            </p>
          </div>
          <div className="flex flex-wrap gap-3">
            <button type="button" onClick={exportRfqPackage} disabled={!draftLines.length} className="inline-flex items-center gap-2 rounded-md bg-[#155fdb] px-4 py-2 text-sm font-bold text-white disabled:cursor-not-allowed disabled:bg-[#9db8e4]">
            ...
            <label className="inline-flex cursor-pointer items-center gap-2 rounded-md border border-[#dfe5ee] bg-white px-4 py-2 text-sm font-bold text-[#071b49]">
              ...
              <input type="file" accept=".xlsx,.xls,.csv,application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" className="hidden" onChange={(event) => void importRfqResponse(event.target.files?.[0])} />
            </label>
          </div>
        </div>
        ...
        <div className="grid gap-3 md:grid-cols-5">
          ...
          <RfqStep complete={rfqReadiness.readyForCustomerQuote} title="5. Ready for PO" detail={rfqReadiness.readyForCustomerQuote ? "Pricing verified" : "Need responses"} />
        </div>
        ...
        {rfqReadiness.missingVerifiedPricingCount ? (
          <div className="rounded-[8px] border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-bold text-amber-800">
            {rfqReadiness.missingVerifiedPricingCount} line{rfqReadiness.missingVerifiedPricingCount === 1 ? "" : "s"} still need vendor quote number and verified unit cost.
          </div>
        ) : null}
      ...
      <form onSubmit={handleAddLine} className="grid gap-6 lg:grid-cols-[1fr_340px]">
        <section className="grid gap-5 rounded-[8px] border border-[#dfe5ee] bg-white p-6 shadow-[0_2px_8px_rgba(15,23,42,0.05)] md:grid-cols-2">
          ...
          <label>
            ...
            <input required list="edit-part-number-suggestions" value={form.partNumber} onChange={(event) => applyCatalogPart(event.target.value)} className="mt-2 w-full rounded-md border border-[#dfe5ee] px-3 py-2 text-sm outline-none focus:border-blue-500" placeholder="Type part number for catalog pricing" />
            <datalist id="edit-part-number-suggestions">
          ...
          <Field label="Manufacturer" placeholder="Enter manufacturer" value={form.manufacturer} onChange={(value) => setForm((current) => ({ ...current, manufacturer: value }))} />
          ...
          <Field label="Vendor Quote Number" placeholder="Optional" value={form.quoteNumber} onChange={(value) => setForm((current) => ({ ...current, quoteNumber: value }))} />
          <Field label="Lead Time" placeholder="Example: 14 days" value={form.leadTime} onChange={(value) => setForm((current) => ({ ...current, leadTime: value }))} />
      ...
      <section className="rounded-[8px] border border-[#dfe5ee] bg-white p-5 shadow-[0_2px_8px_rgba(15,23,42,0.05)]">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            <h3 className="text-lg font-bold text-[#06163d]">Quote Lines</h3>
            <p className="mt-1 text-sm text-[#526179]">Modify any line item field, remove lines, or add new ones before saving changes.</p>
          </div>
          <button type="button" onClick={saveQuote} disabled={isSaving} className="inline-flex items-center gap-2 rounded-md bg-[#155fdb] px-4 py-2 text-sm font-bold text-white disabled:cursor-not-allowed disabled:bg-[#9db8e4]">
            <Save size={17} />{isSaving ? "Saving..." : "Save Changes"}
          </button>
        ...
        <QuoteLinesEditor lines={draftLines} onChange={setDraftLines} emptyMessage="This quote has no lines." showPricingControls={showPricingControls} />
      </section>
    </div>
  );
}
