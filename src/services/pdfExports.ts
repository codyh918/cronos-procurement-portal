...
export async function exportCustomerQuotePdf(quote: CustomerQuote, project?: Project) {
  ...
  const shippingLines = getShippingAddressLines(project)
...
function drawPurchaseOrderTemplateTable(doc: JsPdf, startY: number, po: PurchaseOrder | ProjectPurchaseOrder) {
  ...
  po.lines.map(normalizePurchaseOrderLineForDocument).forEach((line, index) => {
    const cells = [
      ...
      { column: columns[1], text: line.manufacturer || po.vendor || 'N/A' },
...
function drawPurchaseOrderTotal(doc: JsPdf, y: number, total: number) {
  ...
  doc.rect(408, boxY, 82, 30, 'F')
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(10)
  doc.setTextColor(...TEXT)
  doc.text('Total', 424, boxY + 19)
  doc.text(currency(total), 560, boxY + 19, { align: 'right' })
}
...
function getPoComputedTotal(po: PurchaseOrder | ProjectPurchaseOrder) {
