    ...
    [
      {
        ...
        rows: [
          ...
          ['', '', '', '', { value: 'Evans, GA 30809', style: 2 }, '', { value: 'Date:', style: 3 }, { value: formatDateForWorkbook(quote.createdAt), style: 4 }],
          ['', '', '', '', { value: `Cage Code: ${CRONOS_CAGE_CODE}`, style: 2 }, '', { value: 'Expires:', style: 3 }, { value: getQuoteExpirationDateForWorkbook(quote), style: 4 }],
          ['', '', '', '', { value: 'cronosllc.com', style: 2 }],
          [],
...
function worksheetXml(sheet: WorkbookSheet, sheetIndex: number, hasLogo: boolean) {
  ...
  return xmlHeader(`<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">${columnXml}<sheetData>${body}</sheetData>${mergeXml}${drawingXml}</worksheet>`)
}
