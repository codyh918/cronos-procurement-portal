...
import { getAllowedRolesForPath } from '../navigation'
import { fetchSession, getEffectiveRole, normalizeRole } from '../services/auth'
...
const routes = [
  ...
  { path: '/users', name: 'users', component: AdminView },
  { path: '/roles', name: 'roles', component: AdminView },
  { path: '/settings', name: 'settings', component: AdminView },
  { path: '/audit-log', name: 'audit-log', component: AdminView },
  { path: '/account', name: 'account', component: PlaceholderView, meta: { title: 'Account Settings' } },
]
...
router.beforeEach(to => {
  if (to.path.startsWith('/orders/track')) return true

  const allowedRoles = getAllowedRolesForPath(to.path)
  if (!allowedRoles) return true

  const role = normalizeRole(getEffectiveRole(fetchSession()))
