    ...
    <section class="po-lines-panel">
      <div class="po-lines-heading">
        <div>
          ...
          <p>Update vendor order, tracking, shipping, and received status by individual item.</p>
        </div>
            ...
            <tr v-for="line in po.lines" :key="line.id">
              ...
              <td>
                <input
                  ...
                  type="number"
                  :value="String(line.quantityReceived)"
                  @input="updateLine(line.id, { quantityReceived: Number(inputValue($event)) })"
                />
              </td>
              <td>
...
const lineHeadings = ['CLIN', 'Part', 'Description', 'Qty', 'Received', 'Status', 'Vendor Order', 'Tracking', 'Carrier', 'ESD', 'Received Date', 'Notes']
...
async function exportPoPdf() {
  if (!po.value) return
  await exportPurchaseOrderPdf(po.value)
}
