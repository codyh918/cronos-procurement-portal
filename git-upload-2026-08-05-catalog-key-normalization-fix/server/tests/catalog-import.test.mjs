...
test('catalog duplicate lookup is manufacturer-scoped, case-insensitive, and stably paginated', () => {
  const source = fs.readFileSync(new URL('../catalog-api.mjs', import.meta.url), 'utf8')
  assert.match(
    source,
    /select\('id,manufacturer,manufacturer_part_number,current_cost'\)[\s\S]*?\.ilike\('manufacturer', `%\$\{escapeLikePattern\(manufacturer\)\}%`\)[\s\S]*?\.order\('id',[\s\S]*?\.range\(from, from \+ 999\)/,
  )
})
