...
async function importProducts(supabase, parsed, filename, actorId) {
  ...
  const importManufacturers = new Map()
  for (const item of parsed.products) {
    const normalized = string(item.manufacturer).toLowerCase()
    if (normalized && !importManufacturers.has(normalized)) importManufacturers.set(normalized, item.manufacturer)
  }
  for (const manufacturer of importManufacturers.values()) {
    for (let from = 0; ; from += 1000) {
      const result = await supabase
        .from('atlas_products')
        .select('id,manufacturer,manufacturer_part_number,current_cost')
        // Legacy catalog rows may contain surrounding whitespace. The unique
        // index uses btrim(), so use a contains lookup and normalize keys in JS.
        .ilike('manufacturer', `%${escapeLikePattern(manufacturer)}%`)
...
function escapeLikePattern(value) { return string(value).replace(/[\\%_]/g, character => `\\${character}`) }
